...
function validateSortColumn(column) {
    if (!ALLOWED_SORT_COLUMNS.includes(column)) {
        throw new ValidationError(`Invalid sort column: ${column}. Allowed: ${ALLOWED_SORT_COLUMNS.join(', ')}`, {
...
export class SpanRepository {
    ...
    async recordSpanBatch(spans) {
        ...
        if (spans.length > MAX_BATCH_SIZE) {
            throw new ValidationError(`Batch size ${spans.length} exceeds maximum ${MAX_BATCH_SIZE}. ` +
