...
async function example4_MultiLanguage() {
  ...
  const languages: Array<{ lang: string; text: string; voice: TTSVoice }> = [
...
async function example8_ErrorHandling() {
  ...
  // Test 2: Very long text (demonstrating cost awareness)
  const veryLongText = 'Test. '.repeat(10000); // 60k+ characters
...
async function example9_SaveToFiles() {
  ...
  const examples = [
    { name: 'greeting_en', text: 'Hello! Welcome to our service.', voice: 'alloy' as const },
    { name: 'greeting_zh', text: '你好！歡迎使用我們的服務。', voice: 'nova' as const },
    { name: 'notification', text: 'You have a new message.', voice: 'shimmer' as const },
  ];
...
export {
  example1_BasicTTS,
  example2_VoiceComparison,
  example3_StreamingSynthesis,
  example4_MultiLanguage,
  example5_QualityComparison,
  example6_SpeedAdjustment,
  example7_CostTracking,
  example8_ErrorHandling,
  example9_SaveToFiles,
  runAllExamples,
};
