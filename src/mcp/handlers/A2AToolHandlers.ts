...
export class A2AToolHandlers {
  ...
  async handleA2ASendTask(args: unknown): Promise<CallToolResult> {
    ...
    try {
      ...
      throw new Error(
        `Failed to send task to agent ${input.targetAgentId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async handleA2AGetTask(args: unknown): Promise<CallToolResult> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get task ${input.taskId} from agent ${input.targetAgentId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
