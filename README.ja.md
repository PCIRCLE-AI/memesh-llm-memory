...
### Claude Code のための検索可能なプロジェクトメモリ

決定、パターン、コンテキストを — すべてのセッションにわたって記憶します。
...
[インストール](#インストール) • [使い方](#使い方) • [トラブルシューティング](#トラブルシューティング) • [English](README.md)
...
MeMesh Plugin はプロジェクトに**検索可能なメモリ**を提供します。

Claude Code で作業中、MeMesh は重要な決定、アーキテクチャのコンテキスト、学んだ教訓を自動的に保存します。次のセッションで「認証についてどう決めたっけ？」と聞けば、すぐに答えが返ってきます。

**Claude の内蔵メモリとの違いは？**

Claude Code には auto memory と CLAUDE.md がすでにあります — 一般的な設定や指示には最適です。MeMesh はそれに加えて、専用の**プロジェクトメモリ**を提供します。能動的に検索・クエリでき、意味で検索することもできます（キーワードの完全一致だけではありません）。

こう考えてください：
- **CLAUDE.md** = Claude への取扱説明書
- **MeMesh** = プロジェクトが学んだすべてのことの検索可能なノートブック
