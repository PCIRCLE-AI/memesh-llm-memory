...
async function basicExample() {
    ...
    try {
        await rag.indexDocument('TypeScript 是 JavaScript 的超集，添加了靜態類型檢查。它可以編譯成純 JavaScript。', {
        ...
        await rag.indexDocument('React 是一個用於構建用戶界面的 JavaScript 函式庫。它使用虛擬 DOM 來提高性能。', {
        ...
        const documents = [
            {
                ...
                metadata: { source: 'rest-api.md', category: 'api' },
            },
            {
                content: 'GraphQL 是一種 API 查詢語言，由 Facebook 開發。',
                metadata: { source: 'graphql.md', category: 'api' },
            },
            {
                content: 'gRPC 是一個高性能的 RPC 框架，使用 Protocol Buffers。',
                metadata: { source: 'grpc.md', category: 'api' },
            },
            {
                content: 'WebSocket 提供全雙工通訊通道。',
                metadata: { source: 'websocket.md', category: 'api' },
            },
        ];
        await rag.indexDocuments(documents);
...
export { basicExample, batchIndexingExample, hybridSearchExample, advancedSearchExample, };
