...
export class RotationPolicy {
    ...
    `)
            .run(config.name, config.servicePattern, config.maxAgeDays, config.warningDays, config.enforceRotation ? 1 : 0, config.autoArchive ? 1 : 0, config.metadata ? JSON.stringify(config.metadata) : null, now, now);
    ...
    listPolicies() {
        const rows = this.db.prepare('SELECT * FROM rotation_policies ORDER BY name').all();
    ...
    updatePolicy(id, updates) {
      ...
      WHERE id = ?
    `)
            .run(updated.name, updated.servicePattern, updated.maxAgeDays, updated.warningDays, updated.enforceRotation ? 1 : 0, updated.autoArchive ? 1 : 0, updated.metadata ? JSON.stringify(updated.metadata) : null, now, id);
    ...
    assignPolicy(service, account, policyId) {
...
//# sourceMappingURL=RotationPolicy.js.map